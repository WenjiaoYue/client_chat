...
<div class="relative h-[5rem] flex flex-col justify-begin items-center rounded-md">
	<!-- Voice button -->
	<span class="absolute -top-5 text-[#6578aa] text-sm">{displayTimer(voiceTimer)}</span>
	<label class="swap h-full">
	
		<!-- this hidden checkbox controls the state -->
		<input type="checkbox" on:change={toggleRecording} class="hidden"/>
		
		<!-- volume on icon -->
		<img class="h-full swap-on"
	...
	<span class="text-sm text-gray-500">{isRecording ? 'Recording' : 'Record Voice'}</span>
