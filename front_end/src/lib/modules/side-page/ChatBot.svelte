<script lang="ts">
    import { currentTemplate, TemplateCustom, CollectionType } from '$lib/shared/stores/common/Store';
...
<div class="flex h-full">
	<div
		class="h-full w-full sm:mx-5 xl:mx-20 p-5"
	>
        <div class="flex flex-wrap flex-row mb-1 sm:mb-0 justify-between w-full">
            <h2 class="mb-8 text-[1.3rem] font-medium leading-tight text-[#051F61]">My ChatBot</h2>
        </div>
                ...
                {#each $TemplateCustom as opt, i (opt)}
                    ...
                    >
                        <BotTemplateCard
                            ...
                            on:click={() => { currentTemplate.set({collection: CollectionType.Custom, id: i}) }}
                        />
                    </button>
                {/each}
            </div>
        ...
        <div class="flex flex-wrap flex-row mb-1 sm:mb-0 justify-between w-full mt-6">
            <h2 class="mb-8 text-[1.3rem] font-medium leading-tight text-[#051F61]">ChatBot Library</h2>
		</div>
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
			{#each TalkingTemplateLibrary as opt, i}
				<button
                    class="w-full aspect-video rounded-3xl h-full"
