...
<div class="w-full max-w-sm rounded-3xl border border-gray-200 bg-white shadow relative" on:click>
    {#if notLibrary}
        <div class="absolute right-0 top-0 flex flex-row gap-2 items-center">
	...
	<div
		class="flex flex-col items-center rounded-3xl bg-gradient-to-br from-white to-[#bed5fe] py-2"
	>
		...
		<div class="z-10 flex items-center justify-center gap-1">
			<TemplateBook />
			<span class="z-10 text-sm text-black">{knowledge}</span>
		</div>
		<div class="mt-2 flex space-x-3">
			<div
				class="flex-col items-center rounded-full bg-white px-5 py-1 text-center text-sm font-medium text-white focus:ring-4 focus:ring-blue-300
            "
			>
				<button
