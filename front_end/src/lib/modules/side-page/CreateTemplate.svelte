<script lang="ts">
	...
	function handleAvatarUpload(e: CustomEvent<any>) {
		TalkingPhotoCustom.update((options) => {
			return [...options, { name: e.detail.fileName, avatar: e.detail.src }];
		});
		imgBounce = true;
...
<button on:click={() => insertModal.showModal()}>
	<slot>
		<div
			class="flex aspect-video h-full w-full cursor-pointer flex-col justify-center rounded-md border-2 p-2"
		>
			<svg
				...
				/></svg
			>
			<p class="text-black-300 text-sm">Create Template</p>
		</div>
	</slot>
</button>

<dialog class="max-h-[90vh] w-[90vw] p-5" bind:this={insertModal}>
	<div class="mb-7">
		<h3
			class="mb-2 text-center text-[1.3rem] font-medium leading-tight text-[#051F61]"
		>
			Talking Avatar
		</h3>
	...
	<div class="mb-7">
		<h3
			class="mb-4 text-center text-[1.3rem] font-medium leading-tight text-[#051F61]"
		>
			Talking Voice
		</h3>
		<div class="grid grid-cols-4 gap-3 text-[#0F172A]">
			...
			{#each allVoices as opt, i (opt.name + i)}
				<button
					class="aspect-square w-full {`${(i === 0 ) && (VoiceBounce)? "animate-bounce" : ""}`}"
		...
		<div class="grid grid-cols-4 gap-2 text-[#0F172A]">
			...
			{#each allKnowledges as opt, i (opt.name + i)}
				<button
					...
					class={`${(i === 0 ) && (KBounce)? "animate-bounce" : ""}`}
					on:click={() => {
						selectKnowledge = i;
					}}
				>
